...
func TestNewerStmt_arithmatic(t *testing.T) {
	...
	expr := NewNewerStmt(NewTokenScanner(g), nil)
...
func TestNewerStmt_assignation(t *testing.T) {
	...
	vs := NewContext()
	expr := NewNewerStmt(NewTokenScanner(g), vs)
	if err := expr.Execute(); err != nil {
		t.Fatal(err)
	}
...
func TestNewerStmt_objectCall(t *testing.T) {
	...
	vs := NewContext()
	stmt := NewNewerStmt(NewTokenScanner(g), vs)
...
func TestNewerStmt_call(t *testing.T) {
	...
	stmt := NewNewerStmt(NewTokenScanner(g), NewContext())
	if err := stmt.Execute(); err != nil {
		t.Fatal(err)
	}
	if stmt.value.Type != ValueObject {
...
func BenchmarkNewerStmt_arithmatic(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		stmt := NewNewerStmt(NewTokenScanner(g), nil)
		if err := stmt.Execute(); err != nil {
			b.Fatal(err)
		}
