...
func NewNewerStmt(scanner TokenScanner, ctx Context) *newStmt {
	es := exprs([]*expr{
		Assign(scanner),
		Reduction(scanner),
		Or(scanner),
	...
	return &newStmt{expr: es.init()}
}

func (ns *newStmt) Execute() (err error) {
